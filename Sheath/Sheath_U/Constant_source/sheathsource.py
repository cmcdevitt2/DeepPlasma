...
def train_ssbroyden(model,data,maxiter=5000,gtol=1e-12,display_every=1000):
    ...
    neval = [0]
    def objective(x):
        unpack(x); model.net.zero_grad(set_to_none=True)
...
def main():
    ...
    #Compile and train using SSBroyden
    for i in range(0,NumBFGS):
        print(f"\n--- SSBroyden round {i+1}/{NumBFGS} ---")
        result = train_ssbroyden(model,data,maxiter=5000,gtol=1e-12,display_every=1000)
        save_solution(geom, model, "solution0")
        torch.save({"model_state_dict":model.net.state_dict(),"ssbroyden_result":{"nit":result.nit,"nfev":result.nfev,"fun":result.fun,"success":result.success,"message":str(result.message)}},save_path + f"model_SSBroyden_{i+1}.pt")
        dde.saveplot(model.losshistory, model.train_state, issave=True, isplot=True,output_dir= save_path + 'data')
        if i < NumBFGS-1:
            data.resample_train_points(pde_points=True,bc_points=False)
